...
use mod_debatechat\grades\debatechat_gradeitem;
...
use mod_debatechat\local\factories\vault as vault_factory;
use mod_debatechat\local\factories\legacy_data_mapper as legacy_data_mapper_factory;
use mod_debatechat\local\factories\entity as entity_factory;
use mod_debatechat\local\factories\exporter as exporter_factory;
use mod_debatechat\local\factories\manager as manager_factory;
use mod_debatechat\local\factories\builder as builder_factory;
use mod_debatechat\local\factories\url as url_factory;
use mod_debatechat\local\renderers\discussion as discussion_renderer;
use mod_debatechat\local\renderers\discussion_list as discussion_list_renderer;
use mod_debatechat\local\renderers\posts as posts_renderer;
use moodle_page;
use core\output\notification;
...
class renderer {
    ...
    public function __construct(
        ...
        $this->page = $page;
        $this->rendererbase = $page->get_renderer('mod_debatechat');
    }

    /**
     * Create a discussion renderer for the given debatechat and discussion.
     *
     * @param debatechat_entity $debatechat Forum the discussion belongs to
    ...
    public function get_discussion_renderer(
        ...
        $capabilitymanager = $this->managerfactory->get_capability_manager($debatechat);
        ...
        return new discussion_renderer(
            ...
            $baseurl,
            $notifications,
            function($discussion, $user, $debatechat) {
    ...
    public function get_posts_renderer() : posts_renderer {
        return new posts_renderer(
            ...
            'mod_debatechat/debatechat_discussion_posts'
        );
    }
    ...
    public function get_single_discussion_posts_renderer(int $displaymode = null, bool $readonly = false) : posts_renderer {
        ...
        switch ($displaymode) {
            case FORUM_MODE_THREADED:
                $template = 'mod_debatechat/debatechat_discussion_threaded_posts';
                break;
            case FORUM_MODE_NESTED:
                $template = 'mod_debatechat/debatechat_discussion_nested_posts';
                break;
            case FORUM_MODE_NESTED_V2:
                $template = 'mod_debatechat/debatechat_discussion_nested_v2_posts';
                break;
            default;
                $template = 'mod_debatechat/debatechat_discussion_posts';
                break;
        }
        ...
        return new posts_renderer(
            ...
            function($exportedposts, $debatechats, $discussions) use ($displaymode, $readonly, $exportedpostssorter) {
                $debatechat = array_shift($debatechats);
                ...
                $exportedposts = array_map(
                    function($exportedpost) use ($debatechat, $discussionsbyid, $readonly, $seenfirstunread, $displaymode) {
                        $discussion = $discussionsbyid[$exportedpost->discussionid] ?? null;
                        if ($debatechat->get_type() == 'single' && !$exportedpost->hasparent) {
                            ...
                            if (empty($replies)) {
                                ...
                                $sortedreplies = array_map(function($reply) use ($post) {
                                    if (isset($post->author)) {
                                        ...
                                        // debatechat. See above for where it gets unset.
                                        $reply->parentauthorname = get_string('firstpost', 'mod_debatechat');
                                    }
                                    return $reply;
                                }, $sortedreplies);
    ...
    /**
     * Create a posts renderer to render posts in the debatechat search results.
     *
    ...
    public function get_posts_search_results_renderer(array $searchterms) : posts_renderer {
        ...
        return new posts_renderer(
            $this->rendererbase,
            $this->builderfactory->get_exported_posts_builder(),
            'mod_debatechat/debatechat_search_results',
            ...
            function($exportedposts, $debatechatsbyid, $discussionsbyid) use ($searchterms, $urlfactory) {
                ...
                return array_map(
                    function($exportedpost) use (
                        ...
                        $debatechat = $debatechatsbyid[$discussion->get_debatechat_id()];

                        $viewdiscussionurl = $urlfactory->get_discussion_view_url_from_discussion($discussion);
                        $exportedpost->urls['viewdebatechat'] = $urlfactory->get_debatechat_view_url_from_debatechat($debatechat)->out(false);
                        ...
                        $exportedpost->debatechatname = format_string($debatechat->get_name(), true);
                        $exportedpost->discussionname = highlight($highlightwords, format_string($discussion->get_name(), true));
                        $exportedpost->showdiscussionname = $debatechat->get_type() != 'single';
                        ...
                        if ($missingterms) {
                            $strmissingsearchterms = get_string('missingsearchterms', 'debatechat');
    ...
    /**
     * Create a posts renderer to render posts in mod/debatechat/user.php.
     *
     * @param bool $addlinkstocontext Should links to the course, debatechat, and discussion be included?
     * @return posts_renderer
     */
    public function get_user_debatechat_posts_report_renderer(bool $addlinkstocontext) : posts_renderer {
        $urlfactory = $this->urlfactory;

        return new posts_renderer(
            $this->rendererbase,
            $this->builderfactory->get_exported_posts_builder(),
            'mod_debatechat/debatechat_posts_with_context_links',
            function($exportedposts, $debatechatsbyid, $discussionsbyid) use ($urlfactory, $addlinkstocontext) {

                return array_map(function($exportedpost) use ($debatechatsbyid, $discussionsbyid, $addlinkstocontext, $urlfactory) {
                    $discussion = $discussionsbyid[$exportedpost->discussionid];
                    $debatechat = $debatechatsbyid[$discussion->get_debatechat_id()];
                    $courserecord = $debatechat->get_course_record();

                    if ($addlinkstocontext) {
                        $viewdiscussionurl = $urlfactory->get_discussion_view_url_from_discussion($discussion);
                        $exportedpost->urls['viewdebatechat'] = $urlfactory->get_debatechat_view_url_from_debatechat($debatechat)->out(false);
                        $exportedpost->urls['viewdiscussion'] = $viewdiscussionurl->out(false);
                        $exportedpost->urls['viewcourse'] = $urlfactory->get_course_url_from_debatechat($debatechat)->out(false);
                    }
     ...
     *
     * @param debatechat_entity $debatechat The debatechat that the discussions belong to
    ...
    public function get_discussion_list_renderer(
        ...
        switch ($debatechat->get_type()) {
            case 'news':
                if (SITEID == $debatechat->get_course_id()) {
                    $template = 'mod_debatechat/frontpage_news_discussion_list';
                } else {
                    $template = 'mod_debatechat/news_discussion_list';
                }
                break;
            case 'qanda':
                $template = 'mod_debatechat/qanda_discussion_list';
                break;
            default:
                $template = 'mod_debatechat/discussion_list';
        }
        ...
        return new discussion_list_renderer(
            ...
            $capabilitymanager,
            $this->urlfactory,
            debatechat_gradeitem::load_from_debatechat_entity($debatechat),
            $template,
            $notifications,
            function($discussions, $user, $debatechat) {
    ...
    private function get_detailed_discussion_list_renderer(
        ...
        return new discussion_list_renderer(
            ...
            $template,
            $notifications,
            function($discussions, $user, $debatechat) use ($capabilitymanager) {
                ...
                $debatechatdatamapper = $this->legacydatamapperfactory->get_debatechat_data_mapper();
                $debatechatrecord = $debatechatdatamapper->to_legacy_object($debatechat);
                if (debatechat_tp_is_tracked($debatechatrecord, $user)) {
    ...
    public function get_blog_discussion_list_renderer(
        ...
        return $this->get_detailed_discussion_list_renderer($debatechat, 'mod_debatechat/blog_discussion_list');
    }
    ...
    public function get_social_discussion_list_renderer(
        ...
        return $this->get_detailed_discussion_list_renderer($debatechat, 'mod_debatechat/social_discussion_list');
    }
    ...
    public function get_frontpage_news_discussion_list_renderer(
        ...
        return $this->get_detailed_discussion_list_renderer($debatechat, 'mod_debatechat/frontpage_social_discussion_list');
    }
     ...
     * @param bool $hasmultiplediscussions Whether the debatechat has multiple discussions (more than one)
     * @param int $displaymode How should the posts be formatted?
     * @return discussion_renderer
     */
    public function get_single_discussion_list_renderer(
        ...
        $cmid = $debatechat->get_course_module_record()->id;
        $baseurl = $this->urlfactory->get_debatechat_view_url_from_course_module_id($cmid);
        $notifications = array();

        if ($hasmultiplediscussions) {
            $notifications[] = (new notification(get_string('warnformorepost', 'debatechat')))
